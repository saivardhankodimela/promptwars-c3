...
import { Trophy, Flame, Zap, CheckCircle2, User, Users, Compass } from "lucide-react";
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-8 py-6 space-y-8">
      ...
      <div className="flex justify-between items-center">
        ...
        {/* Tab Controls */}
        <div className="flex bg-card-border/30 rounded-xl p-1 border border-card-border/50">
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <h2 className="text-xl font-bold flex items-center gap-2">
              <Zap className="text-secondary h-5 w-5" /> Active Commits
            </h2>
            
            {activeChallenges.length === 0 ? (
              <Card className="text-center p-6 border-dashed border-card-border/80">
                <Compass className="mx-auto text-foreground/30 h-10 w-10 mb-2" />
                  ...
                  <Card key={ac.id} className={isCompleted ? "border-primary/40 bg-primary/5" : ""}>
                    <CardHeader className="pb-2">
                      <div className="flex justify-between items-start">
                        ...
                        {isCompleted && <CheckCircle2 className="text-primary h-5 w-5" />}
                      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <h2 className="text-xl font-bold flex items-center gap-2">
              <Compass className="text-primary h-5 w-5" /> Explore Opportunities
            </h2>
        ...
        <Card className="max-w-3xl mx-auto border border-card-border">
          <CardHeader>
            <CardTitle className="text-xl flex items-center gap-2">
              <Trophy className="text-primary h-5 w-5" /> Global Rankings
            </CardTitle>
                  ...
                  return (
                    ...
                    >
                      <td className="py-4 px-6 text-center font-mono">
                        {entry.rank === 1 ? (
                          <span className="inline-block p-1 bg-yellow-500/20 text-yellow-500 rounded-full">🥇</span>
                        ) : entry.rank === 2 ? (
                          <span className="inline-block p-1 bg-slate-300/20 text-slate-400 rounded-full">🥈</span>
                        ) : entry.rank === 3 ? (
                          <span className="inline-block p-1 bg-amber-700/20 text-amber-700 rounded-full">🥉</span>
                        ) : (
                          `#${entry.rank}`
                        )}
                      </td>
